"""
EXPERIMENT: Solució B — LLM parser + flux determinista (prompt bàsic)
======----------------------------------------------------------------
CLI: python experiment.py --model phi3 --backend local
...
def run(model: str, backend: str = "local") -> dict:
    ...
    print(f"\nSolució B | Backend: {backend} | Model: {model}")
    ...
    return {
        "experiment_id": f"solucio_b_{model_label}",
        "solucio": "B",
        "model": model,
        "backend": backend,
        "data": str(date.today()),
...
if __name__ == "__main__":
    ...
    guardar_resultat(result, ROOT / "model_statistics" / f"solucio_b_{model_label}.json")
    m = result["metriques"]
